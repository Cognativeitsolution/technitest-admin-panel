...
type TradeDialogProps = {
  ...
  submitting?: boolean;
  onCreate: (payload: TradePayload, image: File | null) => Promise<boolean>;
  onUpdate: (
    tradeId: number,
...
export function TradeDialog({
  ...
  useEffect(() => {
    ...
    setDetail(trade?.detail === "string" ? "" : (trade?.detail ?? ""));
    setImage(null);
    setFormError(null);
  ...
  async function handleSave() {
    ...
    const ok =
      isEdit && trade
        ? await onUpdate(trade.id, payload, image)
    ...
    >
      <div className="space-y-4">
        ...
        <div className="flex flex-col gap-2.5">
          <label htmlFor="trade-detail" className="text-[14px] font-medium text-[#111111]">
        ...
        <FileUpload
          key={`${open}-${trade?.id ?? "new"}-${uploadKey}`}
          label="Cover image"
