"use client";

import { Pencil, Star, Trash2 } from "lucide-react";

import { Can } from "@/components/shared/can";
import type { StarRuleRecord } from "@/types/gamification.types";
import { formatDateTime } from "@/lib/utils";

type StarsTableProps = {
  rules: StarRuleRecord[];
  loading?: boolean;
  onEdit: (s: StarRuleRecord) => void;
  onDelete: (s: StarRuleRecord) => void;
};

export function StarsTable({ rules, loading = false, onEdit, onDelete }: StarsTableProps) {
  return (
    <div className="overflow-hidden rounded-2xl border border-[#e8ecf2] bg-white shadow-[0_1px_3px_rgba(16,24,40,0.04)]">
      <div className="overflow-x-auto">
        <table className="w-full min-w-187.5 border-collapse text-left">
          <thead>
            <tr className="bg-[#eef5ff] text-[13px] font-semibold text-[#374151]">
              <th className="px-5 py-3.5">Name</th>
              <th className="px-5 py-3.5">Star Count</th>
              <th className="px-5 py-3.5">Criteria</th>
              <th className="px-5 py-3.5">Status</th>
              <th className="px-5 py-3.5">Last Updated</th>
              <th className="px-5 py-3.5">Actions</th>
            </tr>
          </thead>
          <tbody>
            {loading ? (
              <tr><td colSpan={6} className="px-5 py-10 text-center text-sm text-[#6b7280]">Loading star rules...</td></tr>
            ) : (
              <>
                {rules.map((rule) => (
                  <tr key={rule.id} className="border-t border-[#eef1f6] transition hover:bg-[#fafbfc]">
                    <td className="px-5 py-4 text-sm font-medium text-[#111827]">{rule.name || "--"}</td>
                    <td className="px-5 py-4 text-sm font-semibold text-[#111827]">{rule.stars_count}</td>
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-2">
                        <span className="text-sm text-[#374151]">{rule.criteria}</span>
                        <span className="flex items-center gap-0.5">
                          {Array.from({ length: Math.min(rule.stars_count, 5) }).map((_, i) => (
                            <Star key={i} className="size-4 fill-[#2563eb] text-[#2563eb]" />
                          ))}
                        </span>
                      </div>
                    </td>
                    <td className="px-5 py-4">
                      <span className={`text-sm font-medium ${rule.is_active ? "text-[#16a34a]" : "text-[#9ca3af]"}`}>
                        {rule.is_active ? "Active" : "Inactive"}
                      </span>
                    </td>
                    <td className="px-5 py-4 text-sm text-[#6b7280]">{formatDateTime(rule.updated_at)}</td>
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-1.5">
                        <Can permission="stars:update">
                          <button type="button" aria-label={`Edit ${rule.name || "star rule"}`} onClick={() => onEdit(rule)} className="rounded-lg p-2 text-[#9ca3af] transition hover:bg-[#f3f4f6] hover:text-[#f0a500]">
                            <Pencil className="size-4" />
                          </button>
                        </Can>
                        <Can permission="stars:delete">
                          <button type="button" aria-label={`Delete ${rule.name || "star rule"}`} onClick={() => onDelete(rule)} className="rounded-lg p-2 text-[#9ca3af] transition hover:bg-[#fef2f2] hover:text-[#ef4444]">
                            <Trash2 className="size-4" />
                          </button>
                        </Can>
                      </div>
                    </td>
                  </tr>
                ))}
                {rules.length === 0 ? (
                  <tr><td colSpan={6} className="px-5 py-10 text-center text-sm text-[#6b7280]">No star rules found.</td></tr>
                ) : null}
              </>
            )}
          </tbody>
        </table>
      </div>
    </div>
  );
}
